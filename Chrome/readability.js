...
var readability = {
	 ...
	 **/
	regexps: {
		unlikelyCandidatesRe:   /combx|comment|disqus|foot|menu|nav|rss|shoutbox|sidebar|sponsor|popup|signup|share|cCol/i, //header
		...
		switch(node.tagName) {
			...
			case 'NYT_TEXT':
				if (!node.parentNode.readability) {
					node.parentNode.readability = {"contentScore": 0};								
				}
	...
	**/
	grabArticle: function (preserveUnlikelyCandidates) {
		...
		**/
		...
		dbg("Top Candidates: " + topCandidates.length);
